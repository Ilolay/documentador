...
Public Class frmColaboradores_detajax
Inherits System.Web.Services.WebService
<ScriptMethod()> _ 
<WebMethod()> _ 
Public Function editctrl000009_Get (ByVal prefixText As String, ByVal count As Integer) As String()
...
Dim auxList As New List(Of String)
Dim auxConn As  SqlConnection=gConn_open
Dim auxDA As New SqlDataAdapter("SELECT  DISTINCT EMP.empusername FROM EMP   WHERE (empusername LIKE @prefixText) AND (EMP.baja = 0 OR EMP.baja  IS NULL)",auxConn)
auxDA.SelectCommand.Parameters.Add(New SqlParameter("@prefixText", SqlDbType.Text)).Value ="%" & prefixText & "%"
Dim auxDT As New DataTable
auxDA.Fill(auxDT)
For Each auxRow As DataRow In auxDT.Rows
...
<ScriptMethod()> _ 
<WebMethod()> _ 
Public Function insctrl000009_Get (ByVal prefixText As String, ByVal count As Integer) As String()
Dim auxList As New List(Of String)
Dim auxConn As  SqlConnection=gConn_open
Dim auxDA As New SqlDataAdapter("SELECT  DISTINCT EMP.dsc FROM EMP   WHERE (dsc LIKE @prefixText) AND (EMP.baja = 0 OR EMP.baja  IS NULL)",auxConn)
auxDA.SelectCommand.Parameters.Add(New SqlParameter("@prefixText", SqlDbType.Text)).Value ="%" & prefixText & "%"
