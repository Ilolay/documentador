...
Public Class frmClasificaciones_detajax
...
Dim auxDA As New SqlDataAdapter("SELECT  DISTINCT DOC_CLA.dsc FROM DOC_CLA   WHERE (dsc LIKE @prefixText) AND (DOC_CLA.baja = '1900-01-01T00:00:00' OR DOC_CLA.baja  IS NULL)",auxConn)
...
Dim auxList As New List(Of String)
Dim auxConn As  SqlConnection=gConn_open
Dim auxDA As New SqlDataAdapter("SELECT  DISTINCT DOC_CLA.abrev FROM DOC_CLA   WHERE (abrev LIKE @prefixText) AND (DOC_CLA.baja = '1900-01-01T00:00:00' OR DOC_CLA.baja  IS NULL)",auxConn)
auxDA.SelectCommand.Parameters.Add(New SqlParameter("@prefixText", SqlDbType.Text)).Value ="%" & prefixText & "%"
Dim auxDT As New DataTable
auxDA.Fill(auxDT)
For Each auxRow As DataRow In auxDT.Rows
