...
Public Class frmTiposdeprocesos_det
...
If auxEndOK Then 
     ...
     Else
         Response.Redirect(Session("ROOTWEBURL") & "frmTiposdeprocesos.aspx?_mode_=7&_closea_=0")
     End If
End If
End If
...
Else
    ...
    Dim auxResult as String=auxClass.gSystem_PostAction(3,13,auxDataKeyCod) 
    auxClass.gCluster_LoadStaticTables_DOC_APA(auxDataKeyCod) 
...
Else
    ...
    Dim auxResult as String=auxClass.gSystem_PostAction(3,11,auxDataKeyCod) 
    auxClass.gCluster_LoadStaticTables_DOC_APA(auxDataKeyCod) 
    lblerror.Text &=auxResult
    If auxResult ="" Then
    End If
...
If frmdatos.DataItem IsNot Nothing Then
   Page.Title = "Tipos de procesos| " &  Databinder.Eval(frmdatos.DataItem ,"dsc").ToString
   lblsubtitle.Text = Databinder.Eval(frmdatos.DataItem ,"dsc").ToString
Else
   Page.Title = "Tipos de procesos| Nuevo"
...
Case "1","25"
 If LicParam Is Nothing Then
     gForm_Close()
 Else
     Dim auxMax As Integer = Val(LicParam.gValue_Get("DOC_APA.MAX", -1))
     If auxMax >= 0 Then
         Dim auxConn As clsHrcConnClient = Session("conn")
         auxConn.gComponent_CreateInstance()
         auxConn.gConn_Open()
         If Val(auxConn.gConn_QueryValueInt("SELECT COUNT(*) FROM DOC_APA", 0)) > auxMax Then
             gForm_Close()
         End If
         auxConn.gConn_Close()
     End If
 ...
 Case "25"
   lblsubtitle.Text = "Copiar"
    Try
        ...
        auxConn.Open
        Dim auxDA As New SqlDataAdapter("SELECT DOC_APA.cod,DOC_APA.dsc,DOC_APA.abrev,DOC_APA.baja,DOC_APA.orden,(DOC_APAQSECSID.secdsc) as qsecsiddsc,DOC_APA.qsecdatetime,DOC_APA.qsecsid FROM DOC_APA  LEFT JOIN Q_SECPLOGIN AS DOC_APAQSECSID ON DOC_APAQSECSID.sidcod=DOC_APA.qsecsid  WHERE (DOC_APA.cod=@param1) AND (DOC_APA.baja = '1900-01-01T00:00:00' OR DOC_APA.baja  IS NULL)", auxConn)
...
Dim auxClass As New clscusimDOC
auxClass.Conn.gConn_Open()
Dim auxBagValues As clshrcBagValues = auxClass.gSystem_PostAction_v2(3,Val(Request.QueryString("_mode_")), -1, Nothing)
...
Dim auxClass As New clscusimDOC
auxClass.Conn.gConn_Open()
Dim auxBagValues As clshrcBagValues = auxClass.gSystem_PostAction_v2(3,Val(Request.QueryString("_mode_")),Databinder.Eval(frmdatos.DataItem ,"cod"), Nothing)
If auxBagValues IsNot Nothing Then
     Dim auxValue As Object
End If
auxClass.Conn.gConn_Close()
...
Else
    ...
    Dim auxResult as String=auxClass.gSystem_PostAction(3,12,auxDataKeyCod) 
    auxClass.gCluster_LoadStaticTables_DOC_APA(auxDataKeyCod) 
    lblerror.Text &=auxResult
    If auxResult ="" Then
gForm_Close
...
End Sub
Protected Sub frmTiposdeprocesos_det_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
